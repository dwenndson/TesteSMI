...
{
    public class MeterTicketThread : ITicketThread
    {
        ...
        public MeterTicketThread(MeterDto meter, ClientSettings clientSettings,
            BrokerSettings brokerSettings, IMqttClientConfiguration mqttClientConfiguration,
            IMqttClientMethods mqttClientMethods, Topic topic, CommandRules commandRules, IEventService eventService,
            IForwarderSenderService forwarderSenderService)
        {
            ...
            localClientSettings.ClientId = localClientSettings.ClientId.Replace("serial", meter.Serial);
            localClientSettings.ClientName = localClientSettings.ClientName.Replace("serial", meter.Serial);
            ...
            localTopic.Address = localTopic.Address
                .Replace("{smc-or-meter}", CommandDeviceType.Meter.ToString().ToLower())
                .Replace("{serial}", meter.Serial);
                ...
                do
                {
                    ...
                    foreach (var command in CurrentTicket.Ticket.CommandTickets)
                    {
                        ...
                        do
                        {
                            ...
                            {
                                Console.WriteLine("MIDDLEWARE COM PROBLEMA");
        ...
        public void ReceivedUpdate(object sender, MessageReceivedArgs messageReceivedArgs)
        {
            ...
            {
                switch (ticketCommand.Status)
                {
                    case Status.Waiting:
                        Console.WriteLine("CHEGOU ATUALIZAÇÃO DO COMANDO ESTÁ AGUARDANDO");
                        break;
                    case Status.Executing:
                        Console.WriteLine("CHEGOU ATUALIZAÇÃO DO COMANDO ESTÁ EXECUTANDO");
                        break;
                    case Status.Finished:
                        Console.WriteLine("CHEGOU ATUALIZAÇÃO DO COMANDO ESTÁ FINALIZADO");
                        break;
                    case Status.Failed:
                        Console.WriteLine("CHEGOU ATUALIZAÇÃO DO COMANDO ESTÁ COM FALHA");
                        break;
                }
